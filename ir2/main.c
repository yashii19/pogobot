...
int main(void) {
    ...
    while (1)
    {
		...
		if (microseconds < 1000000 / FQCY) {
 		...
 		/*
 		if (pogobot_stopwatch_get_elapsed_microseconds(&seconds) >= 1000000){

 			printf( "Duration1: %u microseconds \n ", pogobot_stopwatch_get_elapsed_microseconds(&mystopwatch));
 			...
 			pogobot_stopwatch_reset( &seconds );
 			counter = 0;
 			printf( "Duration2: %u microseconds \n ", pogobot_stopwatch_get_elapsed_microseconds(&mystopwatch));
		...
		if ( pogobot_infrared_message_available() ){
			...
			while ( pogobot_infrared_message_available() )
			{
				...
				/*
				if (ir_sender == 0){
					printf("Recu par 0 emetteur. \n ");
				}if (ir_sender == 1){
					printf("Recu par 1 emetteur : %d %d \n ", mr.header._sender_ir_index, old_ir);
				}if (ir_sender == 2){
					printf("Recu par 2 emetteur : %d %d \n", mr.header._sender_ir_index, old_ir);
				}
				*/
