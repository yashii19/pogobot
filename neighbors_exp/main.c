			...
			// si le msg recu est le bon
				...
				if (neighbors_nb == MAX_NEIGHBORS) {
    				...
    				//if (csv_file == NULL) {
        				...
        				//if (csv_file == NULL) {
            				//printf("Failed to create the CSV file.\n");
            				//return 1;  // Or handle the error in an appropriate way
        				//}
        				// Write the CSV header if needed
        				//fprintf(csv_file, "Time (microseconds), Messages Received, Identified Robots, Ticks, Emission Probability, Message Size\n");
    				//}

    				// Write the values to the CSV file
    				//fprintf(csv_file, "%d, %d, %d, %d, %f, %d\n", t_final, msg_counter, neighbors_nb, counter, p_emission, SIZE);
